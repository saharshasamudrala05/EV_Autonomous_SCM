...
import { 
  ...
  Settings, ShieldCheck, DollarSign, Clock, Lock, Truck, Calendar, Download, Sliders,
      ...
      <div className="space-y-6 flex flex-col h-full">
        ...
        <HolographicCard delay={0.7} className="flex-1">
          ...
          <div className="bg-slate-900/50 p-3 rounded border border-slate-700/50">
            ...
            <p className="text-[9px] text-slate-400 leading-relaxed font-mono">All neural decision nodes are currently synchronized. Root cause analysis for Shanghai DC is <span className="text-amber-400">Pending Operator Verification.</span></p>
          </div>
        </HolographicCard>
      </div>
    </div>
...
const DemandForecaster = () => {
  ...
  useEffect(() => {
    fetch('http://localhost:8000/api/forecast/ev-sales')
      .then(res => res.json())
      .then(data => {
        // data.records = raw v4_titan_intelligence_fabric rows (reg_tata, reg_ola, etc.)
        // data.analytics = Bass model KPIs (accuracy, mae, elasticity, etc.)
        setForecast(data.records || []);
        if (data.analytics) setAnalytics(prev => ({ ...prev, ...data.analytics }));
        setLoading(false);
      })
      .catch(e => {
        console.error('Demand Forecaster fetch error:', e);
        setLoading(false);
      });
  }, []);

  // Titan V4 Intelligence Overlay — maker share + battery lead computation
  ...
  const avgPenetration = forecast.length ? forecast.reduce((acc, r) => acc + (parseFloat(r.ev_penetration_rate) || 0), 0) / forecast.length : 0;
  const penetrationPct = Math.min(100, Math.round(avgPenetration * 100));
      ...
      <div className="flex justify-between items-start mb-6">
        <div className="mb-4">
          <div className="flex items-center gap-4 mb-2">
            ...
            <h2 className="text-2xl font-display font-bold text-white tracking-wider uppercase">Demand Intelligence</h2>
          </div>
          <div className="text-xs font-mono text-slate-400 uppercase tracking-widest flex items-center gap-3">
            TITAN V4 CAUSAL ENGINE | BASS DIFFUSION + LIGHTGBM QUANTILE
            <span className="ml-4 text-emerald-400 bg-emerald-950/30 px-2 py-0.5 rounded border border-emerald-500/20 text-[10px]">
              <div className="w-1.5 h-1.5 bg-emerald-400 rounded-full inline-block mr-1 animate-pulse"/> SRC: v4_titan_intelligence_fabric
            </span>
          </div>
        </div>
        <button
          onClick={() => setOverlayOn(o => !o)}
          className={`flex items-center gap-2 px-4 py-2 rounded border font-mono text-[10px] tracking-widest uppercase transition-all ${
            overlayOn ? 'bg-sky-500/20 border-sky-400 text-sky-300' : 'bg-slate-900 border-slate-700 text-slate-400 hover:border-sky-500/50 hover:text-sky-400'
          }`}
        >
          <Activity className="w-3 h-3" />
          {overlayOn ? 'OVERLAY: ON' : 'INTELLIGENCE OVERLAY'}
        </button>
      </div>

      {/* KPI Cards */}
      <div className="grid grid-cols-4 gap-6">
        <KpiCard title="Forecast Accuracy" value={`${analytics.accuracy}%`} trend="↗ Live V4" icon={Crosshair} delay={0.1} />
        <KpiCard title="Error (MAE)" value={analytics.mae.toString()} trend="↘ Optimized" icon={TrendingUp} delay={0.2} />
        <KpiCard title="Adoption Elasticity" value={`${analytics.elasticity}x`} trend="High" icon={Activity} delay={0.3} />
        <KpiCard title="Policy Sensitivity" value={`${analytics.sensitivity}x`} trend="Regional" icon={Sliders} delay={0.4} />
      </div>

      {/* Intelligence Overlay Panel */}
      {overlayOn && (
        <div className="grid grid-cols-3 gap-6">
          <HolographicCard className="col-span-2" delay={0.2}>
            <div className="flex items-center gap-2 mb-4">
              <TrendingUp className="w-4 h-4 text-sky-400" />
              <h3 className="text-[10px] font-mono text-sky-400 uppercase tracking-widest">Maker Volume Intelligence (V4 Causal)</h3>
            </div>
            <div className="space-y-3">
              {makerCols.map((k, i) => {
                const pct = Math.round((makerTotals[i] / totalMakers) * 100);
                return (
                  <div key={k}>
                    <div className="flex justify-between text-[10px] font-mono mb-1">
                      <span className="text-slate-300">{makerLabels[i]}</span>
                      <span style={{color: makerColors[i]}}>{makerTotals[i].toLocaleString()} regs ({pct}%)</span>
                    </div>
                    <div className="h-1.5 w-full bg-slate-900 rounded overflow-hidden">
                      <div className="h-full rounded transition-all duration-700"
                        style={{width: `${pct}%`, backgroundColor: makerColors[i], boxShadow: `0 0 8px ${makerColors[i]}`}} />
                    </div>
                  </div>
                );
              })}
            ...
            <div className="mt-6 pt-4 border-t border-white/10">
              <div className="text-[10px] font-mono text-slate-500 uppercase tracking-widest mb-3">Battery Lead Signal (Supply Precedence — 60-Day Lag)</div>
              <div className="grid grid-cols-3 gap-3">
                {forecast.slice(0, 3).map((r, i) => {
                  const bat = parseFloat(r.battery_lead_signal) || 0;
                  const isRisk = bat < 50;
                  return (
                    <div key={i} className={`p-3 rounded border ${isRisk ? 'border-rose-500/30 bg-rose-950/20' : 'border-emerald-500/20 bg-emerald-950/10'}`}>
                      <div className="text-[9px] font-mono text-slate-400 mb-1 truncate">{r.region_name || 'Node'}</div>
                      <div className={`text-lg font-bold font-mono ${isRisk ? 'text-rose-400' : 'text-emerald-400'}`}>{bat.toFixed(1)}</div>
                      <div className={`text-[8px] font-mono ${isRisk ? 'text-rose-500' : 'text-emerald-600'}`}>{isRisk ? '⚠ STARVATION RISK' : '✓ ADEQUATE'}</div>
                    </div>
                  );
                })}
              </div>
          ...
          <HolographicCard delay={0.3}>
            <div className="flex items-center gap-2 mb-4">
              ...
              <h3 className="text-[10px] font-mono text-sky-400 uppercase tracking-widest">Regional Maturity Index</h3>
            </div>
            <h4 className="text-sm font-display text-white mb-2 uppercase">EV Penetration Rate</h4>
            <div className="flex justify-center my-4 relative">
              <svg viewBox="0 0 120 70" className="w-44 h-24">
                <path d="M 10 65 A 50 50 0 0 1 110 65" fill="none" stroke="#1e293b" strokeWidth="10" strokeLinecap="round"/>
                <path
                  d={`M 10 65 A 50 50 0 0 1 ${10 + Math.cos(Math.PI - (penetrationPct/100)*Math.PI)*(-100)+100} ${65 - Math.sin((penetrationPct/100)*Math.PI)*50}`}
                  fill="none"
                  stroke={penetrationPct > 60 ? '#22c55e' : penetrationPct > 30 ? '#f59e0b' : '#ef4444'}
                  strokeWidth="10" strokeLinecap="round"
                  style={{filter: `drop-shadow(0 0 6px ${penetrationPct > 60 ? '#22c55e' : '#f59e0b'})`}}
                />
                <text x="60" y="62" textAnchor="middle" fill="white" fontSize="18" fontWeight="bold" fontFamily="monospace">{penetrationPct}%</text>
                <text x="60" y="72" textAnchor="middle" fill="#64748b" fontSize="6" fontFamily="monospace">PENETRATION</text>
              </svg>
            </div>
            <div className="space-y-2 mt-2">
              {[{l:'LOW (Emerging)',r:'0–30%',c:'text-rose-400'},{l:'GROWTH (Scaling)',r:'30–60%',c:'text-amber-400'},{l:'MATURE (Inflection)',r:'60%+',c:'text-emerald-400'}].map((t,i)=>(
                <div key={i} className="flex justify-between text-[9px] font-mono">
                  <span className={t.c}>{t.l}</span><span className="text-slate-500">{t.r}</span>
                </div>
              ))}
            </div>
            <div className="mt-4 pt-3 border-t border-white/10 text-[9px] font-mono text-slate-500 uppercase tracking-wider">
              Industrial Inflection: {forecast.filter(r=>(parseFloat(r.reg_industrial)||0)>100).length} regions above threshold
            </div>
          </HolographicCard>
        </div>
      )}

      {/* Main Table + System Perspective */}
      <div className="grid grid-cols-3 gap-6">
        <HolographicCard className="col-span-2 min-h-[400px] flex flex-col" delay={0.5}>
          <div className="flex justify-between items-center mb-6">
            <h3 className="text-sm font-mono text-slate-300 uppercase tracking-widest">Live Demand Signal Stream (Titan V4)</h3>
          </div>
          <div className="flex-1 overflow-auto">
            <table className="w-full text-left text-[10px] font-mono border-collapse">
              <thead>
                <tr className="border-b border-white/10 text-slate-500 uppercase tracking-tighter">
                  <th className="py-2 px-2">Date Key</th>
                  <th className="py-2 px-2">Region</th>
                  <th className="py-2 px-2">Demand</th>
                  {overlayOn && <th className="py-2 px-2">Battery Lead</th>}
                  {overlayOn && <th className="py-2 px-2">EV Pen.</th>}
                  <th className="py-2 px-2">Top Maker</th>
                  <th className="py-2 px-2 text-right">Z-Score</th>
                </tr>
              </thead>
              <tbody>
                {loading ? (
                  <tr><td colSpan="7" className="py-10 text-center text-slate-600">INGESTING TITAN V4 SIGNALS...</td></tr>
                ) : forecast.slice(-12).map((f, i) => {
                  const makers = {Tata:parseFloat(f.reg_tata)||0, Ola:parseFloat(f.reg_ola)||0, Mahindra:parseFloat(f.reg_mahindra)||0};
                  const topMaker = Object.entries(makers).sort((a,b)=>b[1]-a[1])[0];
                  
                  const z = parseFloat(f.demand_z_score) || 0;
                  const zColor = z > 1.5 ? 'text-rose-400' : z < -1.5 ? 'text-amber-400' : 'text-emerald-400';
                  
                  const bat = parseFloat(f.battery_lead_signal) || 0;
                  const evPen = parseFloat(f.ev_penetration_rate) || 0;
                  return (
                    <tr key={i} className={`border-b border-white/5 hover:bg-white/5 transition-colors`}>
                      <td className="py-3 px-2 text-slate-400">{String(f.date_key).slice(0,10)}</td>
                      <td className="py-3 px-2 text-sky-400 font-bold">{f.region_name || '—'}</td>
                      <td className="py-3 px-2 text-white">{(parseFloat(f.target_demand)||0).toFixed(0)} u</td>
                      {overlayOn && <td className="py-3 px-2"><span className={`px-1.5 py-0.5 rounded text-[9px] ${bat<50?'bg-rose-950/40 text-rose-400':'bg-emerald-950/40 text-emerald-400'}`}>{bat.toFixed(1)}{bat<50?' ⚠':''}</span></td>}
                      {overlayOn && <td className="py-3 px-2 text-amber-300">{(evPen*100).toFixed(1)}%</td>}
                      <td className="py-3 px-2"><span className="text-sky-300">{topMaker?`${topMaker[0]} (${topMaker[1].toFixed(0)})`:'—'}</span></td>
                      <td className={`py-3 px-2 text-right font-mono font-bold ${zColor}`}>
                        {z.toFixed(2)}σ
                      </td>
                    </tr>
                  );
        ...
        <HolographicCard delay={0.75} className="flex-1">
          <div className="flex items-center gap-2 mb-4">
            <Activity className="w-4 h-4 text-sky-400" />
            <h3 className="text-[10px] font-mono text-sky-400 uppercase tracking-widest">System Perspective</h3>
          </div>
          <h4 className="text-xl font-display text-white mb-3 italic uppercase">Market Saturation Signals</h4>
          <p className="text-[11px] text-slate-400 leading-relaxed mb-6 border-b border-white/10 pb-6 font-mono">
            The <span className="text-sky-400 font-semibold">Bass Diffusion Model</span> identified a strong <span className="text-sky-400 font-semibold">Imitation Factor (q)</span>. Battery supply lag is a leading indicator of RTO registration peaks.
          </p>
          <div className="bg-rose-950/20 border border-rose-500/20 p-4 rounded mb-8">
            <div className="text-[10px] text-rose-400 font-mono flex items-center gap-2 mb-2 font-bold"><AlertOctagon className="w-3 h-3"/> TITAN V4 RISK SIGNAL</div>
            <p className="text-[10px] text-rose-200/80 leading-relaxed uppercase font-mono">
              Battery lead signal below 50th percentile = 60-day supply constraint ahead of RTO peaks. Monitor Asian imports.
            </p>
          </div>
          <h3 className="text-[10px] font-mono text-slate-500 uppercase tracking-widest mb-4">Adoption Dynamics</h3>
          <div className="space-y-5">
            {[{n:'IMITATION (Q)', v:analytics.q},{n:'INNOVATION (P)', v:analytics.p},{n:'MARKET CAP (M)', v:analytics.m}].map((d,i) => (
              <div key={i}>
                <div className="flex justify-between text-[10px] font-mono mb-2 text-slate-300"><span>{d.n}</span><span>{d.v}%</span></div>
                <div className="h-1 w-full bg-slate-900 rounded"><div className="h-full bg-sky-400 shadow-[0_0_8px_#38bdf8]" style={{width: `${d.v}%`}}/></div>
              </div>
            ))}
          </div>
        </HolographicCard>
...
const AnomalyIntel = () => {
  ...
  useEffect(() => {
    fetch('http://localhost:8000/api/risk/anomalies')
      .then(r => r.json())
      .then(d => {
        setAnomalies(d);
        setLoading(false);
                      ...
                      <td className="py-5 px-4 text-xs">
                        <div className="flex items-center gap-3">
                          <div className="h-1.5 w-20 bg-slate-800 rounded overflow-hidden">
                            <div 
                              className={`h-full shadow-[0_0_8px_currentColor] transition-all duration-1000 ${
                                (item.anomaly_z_score || 0) < 3 ? 'bg-emerald-400' : 
                          ...
                          <span className={`font-mono text-[10px] font-bold ${
                            (item.anomaly_z_score || 0) < 3 ? 'text-emerald-400' : 
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-4 gap-6">
        ...
        <KpiCard title="Stock Health" value="99.2%" trend="↗ +3.1% delta" icon={ShieldCheck} delay={0.3} />
      ...
      <div className="grid grid-cols-3 gap-6">
        ...
        <HolographicCard delay={0.6}>
          ...
          <div className="bg-sky-950/20 p-4 rounded-xl border border-sky-500/30 mb-6">
            ...
            <p className="text-[11px] text-sky-100/80 leading-relaxed font-mono">Current <span className="font-bold text-white">Safety Stock</span> is 12% below the optimal threshold for Battery Modules in the Bangalore facility. The AI suggests an immediate <span className="text-sky-400 font-bold border-b border-sky-400/50">Transfer Order</span> from Mumbai to avoid a Q2 stockout.</p>
          </div>
      ...
      <HolographicCard delay={0.65} className="w-full py-5 px-6">
         ...
         <div className="h-8 flex items-center justify-center text-[10px] font-mono text-slate-600 italic relative overflow-hidden">
            <div className="scanline-overlay opacity-30" />
            No critical border constraints detected in current cycle.
         </div>
      </HolographicCard>

      <HolographicCard delay={0.7} className="w-full">
        <div className="flex items-center gap-4 mb-6">
          <h3 className="text-sm font-mono text-slate-300 uppercase tracking-widest">XGBoost ML Optimization Results</h3>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="text-[9px] font-mono text-slate-500 uppercase tracking-widest border-b border-slate-700/50 bg-black/20">
                <th className="py-4 px-4 font-normal">SKU ID</th>
                <th className="py-4 px-4 font-normal">FACILITY</th>
                <th className="py-4 px-4 text-right font-normal">DAILY DEMAND</th>
                <th className="py-4 px-4 text-right font-normal">DEMAND σ</th>
                <th className="py-4 px-4 text-right font-normal">SAFETY STOCK</th>
                <th className="py-4 px-4 text-right text-sky-400 font-normal">REORDER POINT</th>
                <th className="py-4 px-4 text-right font-normal">EOQ</th>
                <th className="py-4 px-4 text-right text-rose-400 font-normal">MAX STOCK</th>
              </tr>
            </thead>
            <tbody className="text-xs font-mono">
              {[
                {sku: 'BMS_48V_MODULE', fac: 'ATHER_HOSUR', d: 100.6, s: 27.2, ss: 6, rp: 1600, eoq: 1210, m: 2810},
                {sku: 'BMS_48V_MODULE', fac: 'BENGALURU_DC', d: 184.7, s: 25.9, ss: 7, rp: 1488, eoq: 1161, m: 2649},
                {sku: 'BMS_48V_MODULE', fac: 'DELHI_NCR_DC', d: 220.1, s: 29.4, ss: 5, rp: 1762, eoq: 1268, m: 3030},
                {sku: 'BMS_48V_MODULE', fac: 'MAHINDRA_CHAKAN', d: 195.9, s: 27.1, ss: 7, rp: 1549, eoq: 1196, m: 2745},
                {sku: 'BMS_48V_MODULE', fac: 'MUMBAI_PORT', d: 215.7, s: 29.1, ss: 6, rp: 1773, eoq: 1255, m: 3028},
                {sku: 'BMS_48V_MODULE', fac: 'OLA_FUTUREFACTORY', d: 189.8, s: 26.4, ss: 7, rp: 1517, eoq: 1177, m: 2694},
                {sku: 'BMS_48V_MODULE', fac: 'TATA_SANAND', d: 210.8, s: 28.4, ss: 6, rp: 1666, eoq: 1238, m: 2904},
              ].map((row, i) => (
                <tr key={i} className="border-b border-white/5 interactive-row hover:bg-white/5 cursor-pointer">
                  <td className="py-4 px-4 text-slate-300 flex items-center gap-3"><div className="w-5 h-5 rounded-full bg-purple-500/20 border border-purple-500/50 flex items-center justify-center"><Box className="w-3 h-3 text-purple-400"/></div>{row.sku}</td>
                  <td className="py-4 px-4 text-slate-400">{row.fac}</td>
                  <td className="py-4 px-4 text-right text-white font-bold">{row.d}</td>
                  <td className="py-4 px-4 text-right text-rose-400">{row.s}</td>
                  <td className="py-4 px-4 text-right text-amber-400 font-bold">{row.ss}</td>
                  <td className="py-4 px-4 text-right text-sky-400 font-bold bg-sky-950/10">{row.rp}</td>
                  <td className="py-4 px-4 text-right text-emerald-400">{row.eoq}</td>
                  <td className="py-4 px-4 text-right text-rose-500 font-bold bg-rose-950/10 text-glow-red">{row.m}</td>
                </tr>
              ))}
            </tbody>
          </table>
...
const GlobalCorridors = ({ onTriggerOptimizer }) => {
  ...
  const handleOptimize = async () => {
    ...
    try {
      ...
      console.log("TITAN_V4: EXTERNAL PROTOCOL OPENED");
    } catch (e) {
    ...
    setTimeout(async () => {
      try {
        const response = await fetch('http://localhost:8000/api/logistics/routes?origin=Shanghai&destination=Rotterdam&priority=cost');
        if (response.ok) {
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-start mb-6">
        ...
        <div className="flex items-center gap-4 relative z-[100] pointer-events-auto">
          <button 
            ...
            className="px-6 py-3 bg-rose-600 hover:bg-rose-500 text-white font-bold text-[10px] uppercase tracking-[0.2em] rounded transition-all shadow-[0_0_30px_rgba(244,63,94,0.4)] flex items-center gap-3 animate-pulse active:scale-95 border border-rose-400/30 cursor-pointer"
          >
            <Zap className={`w-4 h-4 ${isOptimizing ? 'animate-spin' : ''}`} />
            INITIATE SOVEREIGN OPTIMIZER
          </button>
          <button className="px-4 py-2 bg-slate-900 border border-slate-700 text-sky-400 font-mono text-[10px] rounded tracking-widest flex items-center gap-2 hover:bg-slate-800 transition-colors">
            <Truck className="w-3 h-3"/> GLOBAL FLEET: 1,242 ASSETS
          </button>
        </div>
      </div>

      <div className="grid grid-cols-4 gap-6">
        <KpiCard title="Transit Efficiency" value="94.2%" trend="+2.4% optimized" icon={Clock} delay={0.1} />
        <KpiCard title="Cost per Metric Ton" value="$2.84" trend="-$0.12 savings" icon={DollarSign} delay={0.2} />
      ...
      <div className="grid grid-cols-3 gap-6">
        <HolographicCard className="col-span-2 min-h-[350px] flex flex-col" delay={0.5}>
          ...
          <div className="flex-1 relative w-full border-b border-l border-slate-700/50 ml-4 pb-4">
             <div className="absolute inset-0 flex items-end justify-around px-8">
               {[{h:85, n:'NA-East'}, {h:82, n:'NA-West'}, {h:90, n:'EU-Central'}, {h:75, n:'APAC-South'}, {h:80, n:'APAC-North'}, {h:65, n:'LATAM'}].map((bar, i) => (
                  <div key={i} className="relative w-12 group flex flex-col items-center">
                    <div className="absolute bottom-0 w-full bg-slate-800/80 group-hover:bg-sky-900/50 transition-colors rounded-t border-t border-sky-500/20" style={{height: `${bar.h}%`}}></div>
        ...
        <HolographicCard delay={0.6}>
          ...
          <div className="bg-sky-950/20 p-4 rounded-xl border border-sky-500/30 mb-6 shadow-[inset_0_0_15px_rgba(56,189,248,0.1)]">
            <div className="text-[10px] text-sky-400 font-mono flex items-center gap-2 mb-2 font-bold"><Activity className="w-3 h-3"/> MARKET DRIFT PREDICTION</div>
            <p className="text-[10px] text-slate-300 leading-relaxed font-mono">Logistics models suggest a <span className="text-white font-bold">4.2% increase</span> in sea-freight transit times for the APAC-EU corridor due to seasonal port congestion. Recommendation: <span className="text-sky-400 font-bold border-b border-sky-400/50">Air-Bridge diversion</span> for Class-A inventory.</p>
          </div>
      ...
      <HolographicCard delay={0.7} className="w-full relative">
        ...
        <div className="w-full overflow-x-auto">
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="text-xs font-mono">
               <tr className="border-b border-white/5 interactive-row hover:bg-white/5 cursor-pointer">
                   <td className="py-4 px-4 text-slate-300 flex items-center gap-3"><div className="p-1.5 bg-slate-800 rounded"><Truck className="w-3 h-3 text-slate-400"/></div> #RT-4401</td>
                   <td className="py-4 px-4 text-white font-bold tracking-widest">SHANGHAI → ROTTERDAM</td>
                   <td className="py-4 px-4 text-slate-400 text-[10px]">SEA FREIGHT<br/><span className="text-sky-400">28d target</span></td>
                   <td className="py-4 px-4">
                      <div className="flex items-center justify-center gap-3">
                          <div className="h-1.5 w-32 bg-slate-800 rounded"><div className="h-full bg-sky-400 shadow-[0_0_8px_#38bdf8] w-[94%]"/></div>
                          <span className="text-white font-bold text-[10px]">94%</span>
                      </div>
                   </td>
                   <td className="py-4 px-4 text-right"><span className="text-[9px] bg-emerald-950/40 text-emerald-400 border border-emerald-500/30 px-3 py-1.5 rounded font-bold tracking-widest">ACTIVE</span></td>
                 </tr>
            </tbody>
          </table>
        </div>
  ...
  <div className="space-y-6">
    <div className="flex justify-between items-start mb-6">
      <div className="mb-4">
        ...
        <p className="text-xs font-mono text-slate-400 uppercase tracking-widest flex items-center gap-3">
          ...
          <span className="text-slate-500 text-[10px]">ACTIVE VENDORS: 324 AUDITED</span>
        </p>
      </div>
      <div className="relative">
         ...
         <input type="text" placeholder="PROBE VENDOR DATABASE..." className="bg-slate-900/50 border border-slate-700 text-slate-300 text-[10px] font-mono rounded py-2 pl-8 pr-4 w-64 focus:outline-none focus:border-sky-500/50 transition-colors" />
      </div>
    </div>

    <div className="grid grid-cols-4 gap-6">
      <KpiCard title="Avg. Vendor Score" value="87.4" trend="+2.1 vs prior" icon={Activity} delay={0.1} />
      <KpiCard title="On-Time Rate" value="92.8%" trend="+1.3% improvement" icon={Clock} delay={0.2} />
      <KpiCard title="Quality Benchmark" value="90.2%" trend="+0.8% MoM" icon={ShieldCheck} delay={0.3} />
      <KpiCard title="Risk Exposure" value="Low" trend="Stable" icon={ShieldAlert} delay={0.4} />
    </div>
    ...
    <div className="grid grid-cols-3 gap-6">
      <HolographicCard className="col-span-2 min-h-[350px] flex flex-col" delay={0.5}>
         <div className="flex justify-between items-center mb-8">
          <div>
            <h3 className="text-sm font-mono text-slate-300 uppercase tracking-widest">Strategic Performance Matrix</h3>
            <p className="text-[10px] text-slate-500 mt-1 font-mono">Comparative multi-factor scoring across top-tier audited vendors</p>
          </div>
          <div className="flex gap-4 text-[10px] font-mono bg-black/40 px-3 py-1.5 rounded border border-white/5">
            <span className="flex items-center gap-2 text-sky-400"><div className="w-2 h-2 rounded-full bg-sky-400 shadow-[0_0_8px_currentColor]"/> QUALITY SCORE</span>
            <span className="flex items-center gap-2 text-emerald-400"><div className="w-2 h-2 rounded-full bg-emerald-400 shadow-[0_0_8px_currentColor]"/> DELIVERY SCORE</span>
          </div>
        </div>
        <div className="flex-1 relative w-full border-b border-l border-slate-700/50 ml-4 pb-4">
           {/* Grouped Bar Chart */}
           <div className="absolute inset-0 flex items-end justify-around px-8">
             {[
               {n:'Tech Parts Co', q:94, d:97}, {n:'Global Supply', q:82, d:85}, 
               {n:'Precision Mfg', q:88, d:82}, {n:'FastLogix', q:75, d:89}, 
               {n:'EcoMobility', q:84, d:80}
             ].map((vendor, i) => (
                <div key={i} className="relative w-16 group flex justify-center items-end gap-1">
                  <div className="w-4 bg-sky-500/80 group-hover:bg-sky-400 transition-colors rounded-t shadow-[0_0_10px_rgba(56,189,248,0.2)]" style={{height: `${vendor.q}%`}}></div>
                  <div className="w-4 bg-emerald-500/80 group-hover:bg-emerald-400 transition-colors rounded-t shadow-[0_0_10px_rgba(16,185,129,0.2)]" style={{height: `${vendor.d}%`}}></div>
                  <div className="absolute -bottom-6 text-[8px] font-mono text-slate-500 whitespace-nowrap">{vendor.n}</div>
                </div>
             ))}
           </div>
        </div>
      ...
      <HolographicCard delay={0.6}>
        <h3 className="text-sm font-mono text-slate-300 uppercase tracking-widest mb-6">Intelligence Vector</h3>
        
        <div className="bg-sky-950/20 p-4 rounded-xl border border-sky-500/30 mb-6 shadow-[inset_0_0_15px_rgba(56,189,248,0.1)]">
          <div className="text-[10px] text-sky-400 font-mono flex items-center gap-2 mb-2 font-bold"><Zap className="w-3 h-3"/> STRATEGIC SIGNAL</div>
          <p className="text-[10px] text-slate-300 leading-relaxed font-mono">Vendor <span className="text-white font-bold">TechParts Co.</span> has improved their logistics efficiency by <span className="text-emerald-400">12%</span> following the integration with Nexus-SCM API. Recommend upgrading to <span className="text-sky-400 font-bold">Platinum Tier</span> status for next quarter.</p>
        </div>

        <h4 className="text-[10px] font-mono text-slate-500 uppercase tracking-widest mb-4">GLOBAL ECOSYSTEM VITALITY</h4>
        <div className="space-y-4 mb-6 border-b border-white/10 pb-6">
           {[{n:'SUPPLY CHAIN TRANSPARENCY', v:94, c:'bg-sky-400'}, {n:'ESG COMPLIANCE DELTA', v:88, c:'bg-slate-300'}, {n:'CONTRACTUAL INTEGRITY', v:99, c:'bg-emerald-400'}].map((d,i) => (
              <div key={i}>
                <div className="flex justify-between items-center text-[9px] font-mono mb-1 text-slate-400">
                  <span>{d.n}</span>
                  <span className="text-white">{d.v}%</span>
        ...
        <div className="flex items-center gap-3 bg-slate-900/50 p-3 rounded border border-slate-800">
           <Globe className="w-5 h-5 text-sky-400" />
           <div>
             <div className="text-[10px] font-mono text-sky-400 tracking-widest font-bold">REGIONAL COVERAGE</div>
             <div className="text-[9px] font-mono text-slate-500">14 COUNTRIES</div>
           </div>
        </div>
      </HolographicCard>
    </div>
    ...
    <HolographicCard delay={0.7} className="w-full">
      ...
      <div className="w-full overflow-x-auto">
        <table className="w-full text-left border-collapse">
          ...
          <tbody className="text-xs font-mono">
             <tr className="border-b border-white/5 interactive-row bg-sky-950/10 hover:bg-sky-900/30 cursor-pointer">
                 <td className="py-4 px-4 text-slate-500 font-display text-xl italic font-bold">01</td>
                 <td className="py-4 px-4 text-white">
                    <div className="flex items-center gap-3">
                        <div className="p-2 bg-slate-800 rounded border border-slate-600"><Users className="w-4 h-4 text-sky-400"/></div>
                        <div>
                            <div className="font-bold tracking-wider text-sm">TECHPARTS CO.</div>
                            <div className="text-[9px] text-slate-400">CHINA</div>
                        </div>
                    </div>
                 </td>
                 <td className="py-4 px-4">
                    <div className="flex justify-center gap-6 text-[10px]">
                        <div className="text-center"><div className="text-slate-500 mb-1">ON-TIME</div><div className="text-white font-bold text-sm">97%</div></div>
                        <div className="text-center"><div className="text-slate-500 mb-1">QUALITY</div><div className="text-white font-bold text-sm">94%</div></div>
                    </div>
                 </td>
                 <td className="py-4 px-4 text-center text-3xl font-bold text-sky-400 text-glow-cyan">92</td>
                 <td className="py-4 px-4 text-right"><span className="text-[10px] bg-amber-950/40 text-amber-400 border border-amber-500/30 px-3 py-1.5 rounded font-bold shadow-[0_0_10px_rgba(245,158,11,0.2)] tracking-widest">GOLD PRIORITY</span></td>
               </tr>
          </tbody>
        </table>
      </div>
...
const NetworkReports = () => {
  ...
  const downloadDossier = async () => {
    ...
    try {
        const response = await fetch('http://localhost:8000/api/reports/dossier');
...
const AgenticHub = () => {
  ...
  const triggerSense = async () => {
    setSensing(true);
    try {
      await fetch('http://localhost:8000/api/sense', { method: 'POST' });
  ...
  const approveDecision = async (id) => {
    try {
      await fetch(`http://localhost:8000/api/decisions/approve/${id}`, { method: 'POST' });
      await fetchDecisions();
    } catch (e) { console.error(e); }
  };
...
const MagneticNavItem = ({ item, active, onClick }) => {
  ...
  return (
    <button
      onClick={onClick}
      className={`w-full flex items-center gap-4 px-4 py-3.5 rounded text-[10px] font-mono tracking-widest uppercase transition-all duration-300 relative overflow-hidden group outline-none ${
        active 
